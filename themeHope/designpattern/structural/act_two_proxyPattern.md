...
![img](https://vscodepic.oss-cn-beijing.aliyuncs.com/blog/202407131416451.png)
...
可以看到，在上面在调用对象方法的前后，又重新做了一些其他的工作，相当于是一种扩展。
...
也就是我们可以在代理中，对原来实现了接口的类实现一些扩展操作。
...
Proxy 类中使用频率最高的方法是：newProxyInstance() ，**这个方法主要用来生成一个**代理对象。
...
1. **loader** :类加载器，用于加载代理对象，被代理对象的类加载器。
...
也就是说：**你通过**Proxy类的 **newProxyInstance()** **创建的代理对象在调用方法的时候，实际会调用到实现InvocationHandler **接口的类的 **invoke()**方法。你可以在 invoke() 方法中自定义处理逻辑，比如在方法执行前后做什么事情。
...
![img](https://vscodepic.oss-cn-beijing.aliyuncs.com/blog/202407131416708.png)
...
```java
//这里是获取一个代理对象，参数是我们实现接口的类
SmsService smsService = (SmsService) JdkProxyFactory.getProxy(new SmsServiceImpl());
smsService.send("java");
//如果由新的类实现接口，只需要在这里使用的时候，重新创建一个代理类对象即可，在内部是通过反射机制去调用类的方法
```
...
invoke()方法里面其实就是对我们实现接口的类的方法的扩展。

现在我们对比jdk提供的动态代理和我们刚刚实现的静态代理，刚刚说到静态代理对于新添加的接口需要定义对应的代理类去代理接口的实现类。而上面的测试程序所使用的动态代理规避了这个问题，即我们不需要显示的指定每个接口对应的代理类，有新的接口添加没有关系，只需要在使用的时候传入接口对应的实现类然后返回代理类对象（接口实现类型），然后调用被代理类的方法即可。
...
> https://blog.csdn.net/belongtocode/article/details/136991185
